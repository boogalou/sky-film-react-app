...
const PopularItem = (props) => {
  const { id, title, name, release_date, first_air_date, poster_path, switchPosition } = props;
      ...
      <li>
        <div className="popular-box">
          <div className="popular-b-img">
            <Link to={switchPosition === 'По ТВ' ? `/tvshows/${id}` : `/movie/${id}`}>
          ...
          <div className="popular-b-description">
            <span className={'title'}>
                <Link to={switchPosition === 'По ТВ' ? `/tvshows/${id}` : `/movie/${id}`}>{title || name}</Link>
                </span>
